...
# 
# Die Antworten auf diese Fragen könnten sich je nach Anwendungsgebiet und Datenlage unterscheiden. Es haben sich aber eine Reihe von Anforderungen so durchgesetzt, dass es möglich ist Datenbankensysteme so zu entwickeln dass viele verschiedene Klassen von Datenverarbeitungsproblemen abzudecken. 

# ## Aufbau eines Datenbanksystems 

# Bevor wir die Aufgaben eines Datenbanksystems genauer betrachten, wollen wir nochmal die Begriffe: Datenbank, Datenbanksystem und Datenbankmanagementsystm trennen. Ein Datenbanksystem besteht aus der Datenbank und dem Datenbankmanagementsystem(DBMS). Die Datenbak setzt sich aus den Daten selbst und den Metadaten zusammen, die die Daten beschreiben. Das DBMS ist ein Softwarekomoponente die server-basiert arbeitet. Es ermöglicht den Zugriff auf eine oder mehrere Datenbanken. Somit können mehrere Anwendungen und Nutzer als Klienten eine oder mehrere Datenbankserver nutzen
# 
...
# Die primären Anforderungen an einem Datenbanksystem sind ...
...
# - Integritätssicherung: Das System soll Mehtoden und Instrumente für die Wahrung der Integrität der Daten, das heißt dessen Korrektkeit und Konsistenz bereitstellen.
# - Transaktionen: Das System soll Mehrere DB-Operationen als Funktionseinheit betrachten können und Erfolg bzw. Misserfolg einer solchen Einheit bewerten verbuchen können. 
...
# - Datenschutz: Das System soll Instrumente zum Ausschluss nicht-autorisierter Zugriffe bereitstellen.
# - Datensicherung: Es muss möglich sein Daten nach Systemfehlern wiederherstlelen zu können.
